...
void NotifyHandoverStartEnb(std::string context, uint64_t imsi, uint16_t cellid,
	...
	std::cout << context << " eNB CellId " << cellid
			...
			<< " to CellId " << targetCellId << " time = " << ms << std::endl;
}
...
int main(int argc, char *argv[]) {
	...
	cmd.AddValue("enbTxPowerDbm",
			"TX power [dBm] used by HeNBs (defalut = 46.0)", enbTxPowerDbm);
	...
	for (uint32_t u = 0; u < numberOfUes; ++u) {
		...
		for (uint32_t b = 0; b < numBearersPerUe; ++b) {
			...
			lteHelper->ActivateDedicatedEpsBearer(ueLteDevs.Get(u), bearer,
					tft);
