...
TypeId TapBridge::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::TapBridge").SetParent<NetDevice>().AddConstructor<
					TapBridge>().AddAttribute("Mtu",
					"The MAC-level Maximum Transmission Unit", UintegerValue(0),
					...
					TimeValue(Seconds(0.)),
					MakeTimeAccessor(&TapBridge::m_tStart), MakeTimeChecker()).AddAttribute(
					"Stop",
					"The simulation time at which to tear down the tap device read thread.",
					TimeValue(Seconds(0.)),
					MakeTimeAccessor(&TapBridge::m_tStop), MakeTimeChecker()).AddAttribute(
...
TapBridge::TapBridge() :
		m_node(0), m_ifIndex(0), m_sock(-1), m_startEvent(), m_stopEvent(), m_fdReader(
				0), m_ns3AddressRewritten(false) {
	NS_LOG_FUNCTION_NOARGS ();
...
void TapBridge::Start(Time tStart) {
	...
	m_startEvent = Simulator::Schedule(tStart, &TapBridge::StartTapDevice,
			this);
}
...
void TapBridge::StartTapDevice(void) {
	NS_LOG_FUNCTION_NOARGS ();
...
void TapBridge::StopTapDevice(void) {
	NS_LOG_FUNCTION_NOARGS ();
...
void TapBridge::CreateTap(void) {
	...
	if (pid == 0) {
		...
		if (wantIp && (ipv4 == 0) && m_tapIp.IsBroadcast()
		...
		//
		status = ::execlp(TAP_CREATOR, TAP_CREATOR, // argv[0] (filename)
		...
		// creator process.  Lots of pain to get four bytes. -- keykeykey
		//
		...
		if (m_mode == USE_LOCAL || m_mode == USE_BRIDGE) {
			...
			if (ioctlResult == 0) {
				...
				NS_LOG_INFO(
						"Learned Tap device MacAddr is " << learnedMac << ": setting ns-3 device to use this address");
			...
			if (!m_ns3AddressRewritten) {
				NS_LOG_INFO(
						"Cannot get MacAddr of Tap device: " << m_tapDeviceName << " while in USE_LOCAL/USE_BRIDGE mode: " << std::strerror (errno));
				NS_LOG_INFO(
...
void TapBridge::ForwardToBridgedDevice(uint8_t *buf, ssize_t len) {
	...
	std::cout << "forwarding length = " << len <<  std::endl; //keykeykey
	//
	...
	if (m_mode == USE_LOCAL) {
		...
		if (m_ns3AddressRewritten == false) {
			...
			NS_LOG_LOGIC(
					"Learned MacAddr is " << learnedMac << ": setting ns-3 device to use this address");
...
Ptr<NetDevice> TapBridge::GetBridgedNetDevice(void) {
	NS_LOG_FUNCTION_NOARGS ();
...
uint32_t TapBridge::GetIfIndex(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return m_ifIndex;
}

Ptr<Channel> TapBridge::GetChannel(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return 0;
}
...
Address TapBridge::GetAddress(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return m_address;
}

void TapBridge::SetMode(enum Mode mode) {
	NS_LOG_FUNCTION(mode);
	m_mode = mode;
}

TapBridge::Mode TapBridge::GetMode(void) {
	NS_LOG_FUNCTION_NOARGS ();
	return m_mode;
}
...
uint16_t TapBridge::GetMtu(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return m_mtu;
}

void TapBridge::NotifyLinkUp(void) {
	NS_LOG_FUNCTION_NOARGS ();
	if (!m_linkUp) {
		m_linkUp = true;
...
bool TapBridge::IsLinkUp(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return m_linkUp;
}
...
bool TapBridge::IsBroadcast(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return true;
}

Address TapBridge::GetBroadcast(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return Mac48Address("ff:ff:ff:ff:ff:ff");
}

bool TapBridge::IsMulticast(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return true;
}
...
bool TapBridge::IsPointToPoint(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return false;
}

bool TapBridge::IsBridge(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	//
...
Ptr<Node> TapBridge::GetNode(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return m_node;
}
...
bool TapBridge::NeedsArp(void) const {
	NS_LOG_FUNCTION_NOARGS ();
	return true;
}
