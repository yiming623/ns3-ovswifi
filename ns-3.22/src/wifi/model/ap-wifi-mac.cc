...
TypeId ApWifiMac::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::ApWifiMac").SetParent<RegularWifiMac>().AddConstructor<
					ApWifiMac>().AddAttribute("BeaconInterval",
...
void ApWifiMac::SendAssocResp(Mac48Address to, bool success) {
			...
			if (to == GetAddress()) {
				...
				if (hdr->IsQosData()) {
					if (hdr->IsQosAmsdu()) {
						...
						ForwardUp(packet, from, bssid);//keykeykey
					}
				} else {
...
// To handle Wi-Fi Associate Nodification to Upper Layer, create a dummy packet to userspace to notify the Open vSwitch component.
				Ptr<Packet> p = Create<Packet> ();
				p->AddPaddingAtEnd (6);
				ForwardUp(p, from, hdr->GetAddr1());
// ---
